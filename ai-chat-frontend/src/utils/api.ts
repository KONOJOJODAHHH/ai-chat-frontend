...
export const phraseAPI = {
  getPhrases: async (): Promise<{ preset: PhraseItem[]; custom: PhraseItem[] }> => {
    const res: any = await api.get('/phrases')
    const normalize = (item: any): PhraseItem => ({
      id: String(item?.id ?? ''),
      title: item?.title ?? '',
      content: item?.content ?? '',
    })
    return {
      preset: Array.isArray(res?.preset) ? res.preset.map(normalize) : [],
      custom: Array.isArray(res?.custom) ? res.custom.map(normalize) : [],
    }
  },

  createPhrase: (payload: { title: string; content: string }) =>
    api.post<PhraseItem>('/phrases', payload),

  updatePhrase: (id: string, payload: { title: string; content: string }) =>
    api.put<PhraseItem>(`/phrases/${id}`, payload),
